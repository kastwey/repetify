...
public interface IDeckAppService
{
	...
	Task<Result> DeleteDeckAsync(Guid deckId, Guid userId);
	...
	Task<Result<DeckDto>> GetDeckByIdAsync(Guid deckId, Guid userId);
	...
	/// <param name="userId">The ID of the user who owns the deck to which the card is to be added.</param>
	/// <returns>A task representing the asynchronous operation, with the new Id as the result.</returns>
	Task<Result<Guid>> AddCardAsync(AddOrUpdateCardDto card, Guid deckId, Guid userId);
	...
	/// <param name="userId">The ID of the user who owns the deck to which the card is to be updated.</param>
	/// <returns>A task representing the asynchronous operation.</returns>
	Task<Result> UpdateCardAsync(AddOrUpdateCardDto card, Guid deckId, Guid cardId, Guid userId);
	...
	/// <param name="userId">The ID of the user who owns the deck to which the card is to be deleted.</param>
	/// <returns>A task representing the asynchronous operation</returns>
	Task<Result> DeleteCardAsync(Guid deckId, Guid cardId, Guid userId);
	...
	Task<Result<CardDto>> GetCardByIdAsync(Guid deckId, Guid cardId, Guid userId);
	...
	Task<Result<int>> GetCardCountAsync(Guid deckId, Guid userId);
	...
	Task<Result<IEnumerable<CardDto>>> GetCardsAsync(Guid deckId, Guid userId, int page, int pageSize);
	...
	/// <returns>A task representing the asynchronous operation.</returns>
	Task<Result> ReviewCardAsync(Guid deckId, Guid cardId, Guid userId, bool isCorrect);
	...
	Task<Result<CardsToReviewDto>> GetCardsToReview(Guid deckId, Guid userId, DateTime until, int pageSize, DateTime? cursor);
}
